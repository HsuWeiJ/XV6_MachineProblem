...
void pgtblprint(pagetable_t pagetable, int level,uint64 va_cal) {
  ...
  for(uint64 i = 0; i < 512; i++){
    ...
    if(pte & PTE_V) { // 如果页表项有效
      // 按格式打印页表项
      va=va_cal | i  << PXSHIFT(level);
      //va=va_cal | (unsigned int)255  << 31;
      //va_cal |= i << PXSHIFT(level);
      //char u[1]={241};
      //printf(u);
      printf(" ..");
      ...
      printf("%d: pte %p va %p pa %p V ", i, pte,va, PTE2PA(pte));
      ...
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0){
        // this PTE points to a lower-level page table.
        uint64 child = PTE2PA(pte);
        pgtblprint((pagetable_t)child,level-1,va);
      }
    }
    else if(~(pte & PTE_V) && (pte & PTE_S))
    {
      va=va_cal | i  << PXSHIFT(level);
      printf(" .. .. ..");
      //printf("%d: pte %p va %p blockno %p ", i, pte,va, PTE2BLOCKNO(pte)& 0xFFF);
      printf("%d: pte %p va %p blockno %p ", i, pte,va, PTE2BLOCKNO(pte));
      //printf("%d: pte %p va %p blockno %p ", i, pte,va, pte >> 4);
      if(pte & PTE_R)printf("R ");
      if(pte & PTE_W)printf("W ");
      if(pte & PTE_X)printf("X ");
      if(pte & PTE_U)printf("U ");
...
int madvise(uint64 base, uint64 len, int advice) {
  ...
  case 1:
    if(base+len<=myproc()->sz  )
    {
      ...
      while(cur<base+len)
      {
        pte_t *pte = walk(myproc()->pagetable, cur, 0);
        ...
        else if( !((*pte & PTE_S) || (*pte & PTE_V)))
        {
          if(uvmshouldtouch(cur))
            uvmlazytouch(cur);
        }
        cur += PGSIZE;
      }
    ...
    if(base+len<=myproc()->sz  )
    {
      ...
      while(cur<base+len)
      {
        ...
        if (*pte & PTE_V) 
        {
          ...
          printf("madvise:%p\n",blockno);
          /* Create a PTE from a BLOCKNO and flag bits */
          *pte = BLOCKNO2PTE((uint64)blockno) | PTE_FLAGS(*pte);
          *pte |= PTE_S;
          *pte &= ~PTE_V;
        }
        cur += PGSIZE;
